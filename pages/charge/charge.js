...
Page({
  ...
  onShow(option) {
    my.httpRequest({
        url: app.httpUrl + '/ebike-charge/aliPayXcx/getDevInfo.x', // 该url是自己的服务地址，实现的功能是服务端拿到authcode去开放平台进行token验证
        data: {
  ...
  goCd(e) {
    ...
    if(e.currentTarget.dataset.cdzt == '0'){
      my.navigateTo({ url: '../paycharge/paycharge?id=' + e.currentTarget.dataset.cdczno});
    // 充电状态为充电中，跳转到充电信息页面
    }else if(e.currentTarget.dataset.cdzt == '1'){
      my.navigateTo({ url: '../cdxx/cdxx?id=' + e.currentTarget.dataset.cdczno});
    }
  },
